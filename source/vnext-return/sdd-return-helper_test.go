...
func TestConfigParserRejectsUnknownDuplicateAndSymlink(t *testing.T) {
	...
	if err != nil || values["remote_outbox"] != "/srv/sdd/outbox" { t.Fatalf("valid config failed values=%v err=%v", values, err) }
	if err := os.WriteFile(path, []byte("return_enabled=1\nreturn_enabled=0\n"), 0o600); err != nil { t.Fatal(err) }
	if _, err := parseConf(path, map[string]bool{"return_enabled": true}); err == nil { t.Fatal("duplicate config key accepted") }
	if err := os.WriteFile(path, []byte("return_enabled=1\ncommand=oops\n"), 0o600); err != nil { t.Fatal(err) }
	if _, err := parseConf(path, map[string]bool{"return_enabled": true}); err == nil { t.Fatal("unknown config key accepted") }
	actual := filepath.Join(dir, "actual.conf")
	if err := os.WriteFile(actual, []byte("return_enabled=1\n"), 0o600); err != nil { t.Fatal(err) }
	link := filepath.Join(dir, "link.conf")
	if err := os.Symlink(actual, link); err != nil { t.Fatal(err) }
	if _, err := parseConf(link, map[string]bool{"return_enabled": true}); err == nil { t.Fatal("symlink config accepted") }
}

func TestRemoteOutboxStrictPath(t *testing.T) {
	for _, good := range []string{"/srv/sdd/outbox", "/data/result_v1", "/tmp/a-b.c"} {
		if !safeRemoteOutbox(good) { t.Fatalf("valid outbox rejected: %s", good) }
	}
	for _, bad := range []string{"/", "relative/path", "/tmp/../etc", "/tmp//outbox", "/tmp/out box", "/tmp/out;rm"} {
		if safeRemoteOutbox(bad) { t.Fatalf("unsafe outbox accepted: %s", bad) }
	}
}
...
func TestAtomicRequestAndStateLayout(t *testing.T) {
	...
	if err := rt.ensureLayout(); err != nil { t.Fatal(err) }
	req := fixtureRequest()
	if err := atomicJSON(rt.requestPath(req.ReturnID), req, 0o600); err != nil { t.Fatal(err) }
	if err := rt.writeState(returnState{Schema: stateSchema, ReturnID: req.ReturnID, State: "pending", UpdatedEpoch: time.Now().Unix()}); err != nil { t.Fatal(err) }
	loaded, err := rt.loadRequest(req.ReturnID)
	if err != nil || loaded.DeliveryID != req.DeliveryID { t.Fatalf("request roundtrip failed loaded=%+v err=%v", loaded, err) }
	st, err := rt.loadState(req.ReturnID)
	if err != nil || st.State != "pending" { t.Fatalf("state roundtrip failed state=%+v err=%v", st, err) }
	info, err := os.Stat(rt.requestPath(req.ReturnID)); if err != nil { t.Fatal(err) }
	if info.Mode().Perm()&0o077 != 0 { t.Fatalf("request permissions too broad: %o", info.Mode().Perm()) }
}
...
func TestScanPathOverlapGuard(t *testing.T) {
	...
	if err := rt.ensureLayout(); err == nil { t.Fatal("scan/inbound overlap accepted") }
}
...
func TestReplayIdentityUsesReceiptSHA(t *testing.T) {
	...
	a, _ := fileSHA256(one); b, _ := fileSHA256(two)
	if a == b { t.Fatal("different receipts produced same fixture digest") }
}

func TestExactMarkerLine(t *testing.T) {
	path := filepath.Join(t.TempDir(), "result.txt")
	if err := os.WriteFile(path, []byte("before\nRESULT: PASS\nafter\n"), 0o600); err != nil { t.Fatal(err) }
	ok, err := fileHasExactLine(path, "RESULT: PASS"); if err != nil || !ok { t.Fatalf("exact marker not found ok=%v err=%v", ok, err) }
	ok, err = fileHasExactLine(path, "PASS"); if err != nil || ok { t.Fatalf("substring incorrectly accepted ok=%v err=%v", ok, err) }
}

func mustJSON(t *testing.T, value any) []byte {
	t.Helper(); data, err := json.Marshal(value); if err != nil { t.Fatal(err) }; return data
}
