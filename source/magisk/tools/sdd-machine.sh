...
emit_capabilities(){
  commands='version capabilities status targets target-test dispatch delivery-status delivery-wait requeue logs doctor chatgpt-context snapshot explain config install-termux bridge-status'
  if [ "$FORMAT" = json ]; then
    printf '{"schema":"sdd-cli-capabilities-v2","cliSchema":%s,"chatgptContextSchema":%s,"machineOutput":{"env":true,"json":true,"noPrompt":true},"verifyOwnership":{"dispatcher":true,"remoteShaRequired":true,"bashFallback":"fail_closed","pythonDelivery":"unsupported"},"commands":' "$SDD_CLI_SCHEMA" "$SDD_CHATGPT_CONTEXT_SCHEMA"; json_string "$commands"; printf '}\n'
  else
    echo "schema=sdd-cli-capabilities-v2"; echo "cli_schema=$SDD_CLI_SCHEMA"; echo "chatgpt_context_schema=$SDD_CHATGPT_CONTEXT_SCHEMA"
    echo "machine_env=yes"; echo "machine_json=yes"; echo "no_prompt=yes"; echo "verify_owner=dispatcher"; echo "remote_sha_required=yes"
...
emit_chatgpt_context(){
  ...
  if [ "$FORMAT" = json ]; then
    ...
    echo "schema=SDD_CHATGPT_CONTEXT_V1"; echo "cli_schema=$SDD_CLI_SCHEMA"; echo "version=$version"; echo "versionCode=$version_code"; echo "health=$health"
    echo "main_pid_ok=$main_ok"; echo "watcher_pid_ok=$watcher_ok"; echo "watchdog_pid_ok=$watchdog_ok"; echo "inflight_bytes=$inflight"; echo "event_pending=$event_pending"
    ...
    echo "inflight_records=$inflight_records"; echo "fail_records=$fail_records"; echo "quarantine_records=$quarantine_records"; echo "recent_fail_lines=$recent_fail"; echo "recent_warn_lines=$recent_warn"
...
emit_explain(){
  ...
  case "$code" in
    ...
    DELIVERY_TIMEOUT|delivery_timeout) reason='Delivery did not reach verified complete state in time.'; action='Run sdd delivery status <file> before requeueing.' ;;
    ...
    *) reason='Unknown or unmapped result code.'; action='Run sdd doctor --chatgpt and sdd chatgpt-context.' ;;
  esac
