...
import_bundle_if_needed(){
  ...
  if [ -d "$BUNDLE_DIR/ssh" ]; then
    for x in id_drop_dispatch_ed25519 id_ed25519 id_rsa known_hosts ssh-config.dispatch; do
      [ -f "$BUNDLE_DIR/ssh/$x" ] && { src_sig=$($CKSUM_BIN "$BUNDLE_DIR/ssh/$x" 2>/dev/null); dst_sig=""; [ -f "$SSH_DIR/$x" ] && dst_sig=$($CKSUM_BIN "$SSH_DIR/$x" 2>/dev/null); if [ -n "$src_sig" ] && [ "$src_sig" != "$dst_sig" ]; then tmp="$SSH_DIR/.$x.import.$$"; if $CP_BIN -f "$BUNDLE_DIR/ssh/$x" "$tmp" >/dev/null 2>&1 && $CHMOD_BIN 600 "$tmp" >/dev/null 2>&1 && $MV_BIN -f "$tmp" "$SSH_DIR/$x" >/dev/null 2>&1; then :; else $RM_BIN -f "$tmp" >/dev/null 2>&1 || true; log "WARN bundle_ssh_atomic_refresh_failed file=$x"; fi; fi; } || true
    done
  fi
